...
class Mpesa {
     ...
     * @param $ResponseType | This parameter specifies what is to happen if for any reason the validation URL is not reachable. Only two values are allowed: Completed or Cancelled.
     * @param $ConfirmationURL | This is the URL that receives the confirmation request from API upon payment completion.
     * @param $ValidationURL | This is the URL that receives the validation request from API upon payment submission.
     * @return mixed
     */
		public static function registerurl($ShortCode, $ResponseType, $ConfirmationURL, $ValidationURL){
				$live=MPESA_API_ENV;
        if( $live =="live"){
    ...
    /**
     *Use this function to format the phone number to a valid MSISDN
     */
